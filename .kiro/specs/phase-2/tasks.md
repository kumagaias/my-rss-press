...
**受け入れ基準**:
- [x] `fast-check` がインストールされている
- [x] 言語検出のプロパティテストが実装されている（プロパティ 1, 2, 3）
- [x] 言語フィルタリングのプロパティテストが実装されている（プロパティ 4, 5）
- [x] 検索フィルタリングのプロパティテストが実装されている（プロパティ 6）
- [x] 日付検証のプロパティテストが実装されている（プロパティ 7, 8）
- [x] 過去の新聞キャッシングのプロパティテストが実装されている（プロパティ 9）
- [x] 日付ベース記事フィルタリングのプロパティテストが実装されている（プロパティ 10）
- [x] 要約生成のプロパティテストが実装されている（プロパティ 11, 12）
- [x] 画像存在のプロパティテストが実装されている（プロパティ 13）
- [x] クリーンアップロジックのプロパティテストが実装されている（プロパティ 14）
- [x] データ永続性のプロパティテストが実装されている（プロパティ 15, 16, 17）
- [x] ローディングアニメーションのプロパティテストが実装されている（プロパティ 18）
- [x] 各テストが設計書のプロパティを明示的に参照している
- [x] `make test-unit` が成功する（Frontend: 282テスト, Backend: 93テスト）
...
**実装メモ**:
- `fc.date()`の問題を修正（無効な日付生成を防ぐため`fc.integer().map()`を使用）
- 非同期プロパティテストに`fc.asyncProperty`を使用
- `fc.option()`で`null`の代わりに`undefined`を返すよう設定（`{ nil: undefined }`）
- 全18プロパティのテストを実装し、各100回以上の反復で検証
...
**受け入れ基準**:
- [x] 言語フィルター選択と新聞フィルタリングの E2E テストが実装されている
- [x] フリーワード検索機能の E2E テストが実装されている
- [x] 日付ナビゲーション（前日/翌日）の E2E テストが実装されている
- [x] 初回アクセス時の過去の新聞生成の E2E テストが実装されている
- [x] 2回目アクセス時の過去の新聞取得の E2E テストが実装されている
- [x] 未来の日付の拒否の E2E テストが実装されている
- [x] 古い日付（> 7日）の拒否の E2E テストが実装されている
- [x] 生成中のローディングアニメーション表示の E2E テストが実装されている
- [x] 新聞内の要約表示の E2E テストが実装されている
- [x] 著作権フリー画像のフォールバックの E2E テストが実装されている
- [x] Page Object Model パターンを使用している（HistoricalNewspaperPage）
- [ ] `npm run test:e2e` が成功する（実装完了、実行は別途確認）
...
**実装メモ**:
- HistoricalNewspaperPage（Page Object）を作成
- 7つのE2Eテストファイルを実装：
  1. language-filter.spec.ts（言語フィルター、2テスト）
  2. search.spec.ts（フリーワード検索、3テスト）
  3. date-navigation.spec.ts（日付ナビゲーション、4テスト）
  4. historical-newspaper.spec.ts（過去の新聞生成・取得・日付検証、5テスト）
  5. loading-animation.spec.ts（ローディングアニメーション、4テスト）
  6. summary-display.spec.ts（要約表示、5テスト）
  7. image-fallback.spec.ts（画像フォールバック、5テスト）
- 合計28のE2Eテストケースを実装
- 全ての受け入れ基準をカバー
- 実際の実行は実装完了後に確認が必要（バックエンドAPIとの統合が必要）
...
**受け入れ基準**:
- [x] `product.md` に Phase-2 の機能が追加されている
- [x] `tech.md` に言語検出、要約生成、過去の新聞の実装詳細が追加されている
- [x] `structure.md` に新しいファイルとディレクトリが追加されている
- [x] 更新履歴セクションに Phase-2 の変更が記録されている
- [x] すべてのドキュメントが一貫性を保っている
...
**更新内容**:
- **product.md**: 
  - Core Featuresに9-15の新機能を追加（言語検出、検索、過去の新聞、要約、日付ナビゲーション、ローディング、クリーンアップ）
  - Update Historyに2025-12-11のエントリを追加
- **tech.md**:
  - Phase 2セクションを追加（Language Detection、Summary Generation、Historical Newspaper、Cleanup Service）
  - 各サービスの実装詳細とコード例を記載
- **structure.md**:
  - Backend servicesに4つの新サービスを追加
  - Frontend componentsにPhase 2のコンポーネントを追加
  - E2E testsの詳細構造を追加（7つのspecファイル）
  - Property testsを追加
